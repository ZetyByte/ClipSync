...
const Chat: React.FC<ChatProps> = ({socket, peerPublicKey, setChatStarted, setHistory, history, setError, disconnectClient}) => {
    ...
    const sendMessage = async (text: string) => {
        ...
        if (text.length > messageLengthLimit) {
            alert('Message length limit is ' + messageLengthLimit + ' characters.');
        } else {
            setHistory((prev: any) => [...prev, 'You: ' + text]);
            let encryptedString = ""; 
            while (text.length > encrptionLimit) {
                encryptedString = encryptedString + await crpt.encrypt(text.substring(0, encrptionLimit), peerPublicKey!);
                text = text.substring(encrptionLimit);
            }
